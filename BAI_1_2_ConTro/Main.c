...
void conTroC()
{
	int b = 5;//Khai báo 1 biến tên là b có kiểu dữ liệu là số nguyên có giá trị = 5

	int* point;//Khai báo 1 con trỏ tên là point 

	int a;
	printf("Dia Chi cua b co kieu HEX %p", &b);//Dấu & giúp lấy địa chỉ ô nhớ
	printf("Gia tri cua b %d", b);
}
void khaiBaoConTro()
{
	int* prt1;//Khai báo 1 con trỏ có tên prt1 trỏ đến vùng nhớ có kieru int
	double* prt2;//Trỏ đến vùng nhớ có kiểu double
	float* prt3;
	char* prt4;
	int* prt5, a;//Khai báo 1 con trỏ có ptr5 trỏ đến vùng nhớ có kiểu int và khai báo 1 biến a có kiểu int

	//Lưu ý: Mỗi con trỏ nên khai báo trên 1 dòng code
	/*
	 * Ví dụ 1: Khai báo 1 biến không nhất thiết biến phải có giá trị nhưng nó sẽ phải địa chỉ.
	 */
	int b = 9;
	int c = 5;
	printf("-----------------------------------\n");
	...
	printf("Gia tri cua c o ram %d \n",c);
	//%p sử dụng để in một địa chỉ có kiểu HEX
	printf("-----------------------------------\n");
	...
	ptr6 = &b;//Gán địa chỉ của b cho con trỏ ptr6
	//Có thể hiểu con trỏ ptr6 đang trỏ đến địa chỉ của ô nhớ b

	//Muốn xem giá trị của địa chỉ con trỏ trỏ đến dùng toán tử *
	printf("-----------------------------------\n");
	printf("Dia chi cua con tro ptr6 dang tro den b la %p \n", ptr6);
	printf("Gia tri cua con tro ptr6 dang tro den b la %d \n", *ptr6);
	printf("Dia chi cua b co kieu hex la %p \n", &b);
	printf("-----------------------------------\n");

	//Khi tác động đến con trỏ ptr6 = giá trị thì b sẽ bị thay đổi
	printf("-----------------------------------\n");
	*ptr6 = 12;//Gán giá trị mới vào ô địa chỉ mà con trỏ đang nắm giữ
	printf("Gia tri cua b sau khi con tro *ptr6 = 12 thi b = %d \n", b);
	printf("-----------------------------------\n");

	//Con trỏ trỏ đến con trỏ
	ptr7 = ptr6;//Gán địa chỉ
	*ptr7 = 99;
	printf("----------Con tro tro den con tro\n");
	printf("Dia chi con tro ptr6 dang tro den la %p \n", ptr6);
	printf("Dia chi con tro ptr7 dang tro den la %p \n", ptr7);
	printf("Gia tri con tro ptr7 dang tro den la %d \n", *ptr7);
	printf("Gia tri cua b la %d \n", b);
}
void sizeContro()
{
	// kích thước là 4 dành cho máy 32-bit || 8 dành cho máy 64-bit
